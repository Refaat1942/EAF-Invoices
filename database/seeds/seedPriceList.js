...
async function importPriceListPayload(payload, actor = null, options = {}) {
  ...
  if (options.replaceExisting && list.code) {
    await query('DELETE FROM price_lists WHERE code = $1', [list.code]);
  }

  const existing = await query('SELECT id FROM price_lists WHERE code = $1', [list.code]);
  let priceListId;
  ...
  for (const svc of services) {
    ...
    await query('DELETE FROM service_price_components WHERE service_id = $1', [serviceId]);
    await query('DELETE FROM service_price_tiers WHERE service_id = $1', [serviceId]);
    ...
    await syncStayTypeFromService(svc, Number(svc.price) || 0);
  }
...
async function syncStayTypeFromService(service, price) {
  const stayNames = [
    'رعاية مركزة',
    'رعاية تلطيفية',
    'VIP',
  ...
  await query(
    `UPDATE stay_types SET daily_rate = $2 WHERE name = $1`,
    [match, price]
  );
}

async function getPriceListStats(priceListId) {
